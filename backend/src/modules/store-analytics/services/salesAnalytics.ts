...
function groupPerDate(
  orders: any[],
  resolution: DateResolutionType,
  ...
  return orders.reduce((accumulator, order) => {
    ...
    if (!accumulator[truncatedDate.toISOString()]) {
      if (resolution == DateResolutionType.Day) {
        accumulator[truncatedDate.toISOString()] = {
          date: new Date(new Date(order.created_at).setHours(0, 0, 0, 0)),
          total: 0,
        };
      } else {
        accumulator[truncatedDate.toISOString()] = {
          date: new Date(
            new Date(new Date(order.created_at).setDate(1)).setHours(0, 0, 0, 0)
          ),
          total: 0,
        };
      }
    }
    accumulator[truncatedDate.toISOString()].total +=
      order.totals.original_order_total * Math.pow(10, decimalMultiplier);
    return accumulator;
  }, {});
}
...
export default class SalesAnalyticsService {
    ...
    if (orderStatusesAsStrings.length) {
      ...
      if (startQueryFrom) {
        if (dateRangeFromCompareTo && from && to && dateRangeToCompareTo) {
          const previousOrders = ordersWithSummary.filter(
            (order) => order.created_at < from
          );
          const currentOrders = ordersWithSummary.filter(
            (order) => order.created_at >= from
          );
          const resolution = calculateResolution(from);
          const groupedCurrentOrders = groupPerDate(
            currentOrders,
            resolution,
            decimalDigits
          );
          const groupedPreviousOrders = groupPerDate(
            previousOrders,
            resolution,
            decimalDigits
          );
          const currentSales: SalesHistory[] =
            Object.values(groupedCurrentOrders);
          const previousSales: SalesHistory[] = Object.values(
            groupedPreviousOrders
          );
          return {
            ...
            current: currentSales.sort(
              (a, b) => a.date.getTime() - b.date.getTime()
            ),
            previous: previousSales.sort(
              (a, b) => a.date.getTime() - b.date.getTime()
            ),
          };
        }
  ...
  async getSalesChannelsPopularity(
    ...
    dateRangeFromCompareTo?: Date,
    dateRangeToCompareTo?: Date
  ): Promise<OrdersSalesChannelPopularityResult> {
    const orderStatusesAsStrings = Object.values(orderStatuses);
    if (orderStatusesAsStrings.length) {
      if (dateRangeFromCompareTo && from && to && dateRangeToCompareTo) {
        const resolution = calculateResolution(from);
        ...
        const finalOrders: OrdersSalesChannelPopularityResult =
          ordersCountBySalesChannel.reduce((acc, entry) => {
      ...
      if (startQueryFrom) {
        ...
        const finalOrders: OrdersSalesChannelPopularity[] =
          ordersCountBySalesChannel.map((order) => {
            return {
              date: order.date,
  ...
  async getRegionsPopularity(
    orderStatuses: OrderStatus[],
    from?: Date,
    to?: Date,
    dateRangeFromCompareTo?: Date,
    dateRangeToCompareTo?: Date
  ): Promise<OrdersRegionsPopularityResult> {
    const orderStatusesAsStrings = Object.values(orderStatuses);
    if (orderStatusesAsStrings.length) {
      if (dateRangeFromCompareTo && from && to && dateRangeToCompareTo) {
        const resolution = calculateResolution(from);
        ...
        const finalOrders: OrdersRegionsPopularityResult =
          ordersCountByRegion.reduce((acc, entry) => {
            const type = entry.type;
            const date = entry.date;
            const orderCount = entry.orderCount;
      ...
      if (startQueryFrom) {
        ...
        const finalOrders: OrdersRegionsPopularity[] = ordersCountByRegion.map(
          (order) => {
            return {
              date: order.date,
              orderCount: order.ordercount,
  ...
  async getRefunds(
    ...
    if (dateRangeFromCompareTo && from && to && dateRangeToCompareTo) {
      ...
      const previousRefunds = refunds.find(
        (refund) => refund.type == "previous"
      );
