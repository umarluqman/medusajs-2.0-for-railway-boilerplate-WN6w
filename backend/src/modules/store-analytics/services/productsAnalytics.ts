...
export default class ProductsAnalyticsService {
  ...
  async getTopVariantsByCount(
    ...
    if (orderStatusesAsStrings.length) {
      if (from && to) {
        const rawVariantsSumInLineItemsInOrders = await this.pgConnection(
          "order"
        )
        ...
        const variantsSumInLinteItemsInOrders =
          rawVariantsSumInLineItemsInOrders as any;
  ...
  async getOutOfTheStockVariants(
    limit?: number
  ): Promise<OutOfTheStockVariantsCountResult> {
    ...
    const query = this.pgConnection("product")
      ...
      .join(
        "inventory_item",
        "product_variant_inventory_item.inventory_item_id",
        "inventory_item.id"
      )
      .join(
        "inventory_level",
